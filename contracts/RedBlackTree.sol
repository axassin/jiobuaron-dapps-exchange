...
library RedBlackTree {
    ...
    function getItem(Tree storage tree, uint256 id) public constant returns (uint256 parent, uint256 left, uint256 right, uint value, bool red) {
        require(id != 0);
    ...
    function find(Tree storage tree, uint value) public constant returns (uint256 parentId) {
        uint256 id = tree.root;
        parentId = id;
        while (id != 0) {
    ...
    function insert(Tree storage tree, uint256 id, uint value) internal {
        uint256 parent = find(tree, value);
        placeAfter(tree, parent, id, value);
    }

    function placeAfter(Tree storage tree, uint256 parent, uint256 id, uint value) internal {
        Item memory item;        
        item.value = value;
        item.parent = parent;
        item.red = true;
    ...
    function insert1(Tree storage tree, uint256 n) private {
        uint256 p = tree.items[n].parent;
        if (p == 0) {
            tree.items[n].red = false;
        } else {
            if (tree.items[p].red) {
                uint256 g = grandparent(tree, n);
                uint256 u = uncle(tree, n);
    ...
    function insert2(Tree storage tree, uint256 n) internal {
        uint256 p = tree.items[n].parent;
        uint256 g = grandparent(tree, n);
    ...
    function remove(Tree storage tree, uint256 n) internal {
        uint256 successor;
        uint256 nRight = tree.items[n].right;
        uint256 nLeft = tree.items[n].left;
        if (nRight != 0 && nLeft != 0) {
            successor = nRight;
            while (tree.items[successor].left != 0) {
                successor = tree.items[successor].left;
            }
            uint256 sParent = tree.items[successor].parent;
            if (sParent != n) {
    ...
    function delete1(Tree storage tree, uint256 n) private {
        uint256 p = tree.items[n].parent;
        if (p != 0) {
            uint256 s = sibling(tree, n);
            if (tree.items[s].red) {
    ...
    function delete2(Tree storage tree, uint256 n) private {
        uint256 s = sibling(tree, n);
        uint256 p = tree.items[n].parent;
        uint256 sLeft = tree.items[s].left;
        uint256 sRight = tree.items[s].right;
        if (!tree.items[p].red && 
            !tree.items[s].red && 
    ...
    function grandparent(Tree storage tree, uint256 n) private returns (uint256) {
        return tree.items[tree.items[n].parent].parent;
    }

    function uncle(Tree storage tree, uint256 n) private returns (uint256) {
        uint256 g = grandparent(tree, n);
        if (g == 0)
            return 0;
    ...
    function sibling(Tree storage tree, uint256 n) private returns (uint256) {
        uint256 p = tree.items[n].parent;
        if (n == tree.items[p].left) {
    ...
    function rotateRight(Tree storage tree, uint256 n) private {
        uint256 pivot = tree.items[n].left;
        uint256 p = tree.items[n].parent;
    ...
    function rotateLeft(Tree storage tree, uint256 n) private {
        uint256 pivot = tree.items[n].right;
        uint256 p = tree.items[n].parent;
        tree.items[pivot].parent = p;
        if (p != 0) {
            if (tree.items[p].left == n) {
